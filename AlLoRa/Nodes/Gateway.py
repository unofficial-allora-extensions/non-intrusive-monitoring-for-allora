...
class Gateway(Requester):
    ...
    def __init__(self, connector=None, config_file="LoRa.json", debug_hops=False, 
        ...
        # We charge "controladores.json", the file that contains the correspondence between a sensor node's MAC address,
        # and the MAC address of its assigned controller
        self.controllers_map = {}
        self.controllers_reverse_map = {}

        try:
            with open("controladores.json", "r") as f:
                self.controllers_map = json.load(f)

            # We create a reverse map: controller → sensor
            self.controllers_reverse_map = {
                v: k for k, v in self.controllers_map.items()
            }

        except Exception as e:
            if self.debug:
                print("Error loading controllers map:", e)
            ...
            for ep in sorted(self.digital_endpoints, key=lambda x: next_check_times[x.get_mac_address()]):
                if current_time >= next_check_times[ep.get_mac_address()]:
                    try:
                        ...
                        next_check_times[ep.get_mac_address()] = time() + (ep.asking_frequency * 1000)
